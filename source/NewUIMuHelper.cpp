...
bool SEASON3B::CNewUIMuHelper::Create(CNewUIManager* pNewUIMng, int x, int y)
{
	...
	m_pSkillSettingWnd = new CNewUIMuHelper_Skill;
	m_pSkillSettingWnd->Create(pNewUIMng, x - 192, y);
...
void SEASON3B::CNewUIMuHelper::Release()
{
	...
	SAFE_DELETE(m_pSkillSettingWnd);
}
...
void SEASON3B::CNewUIMuHelper::InitButtons()
{
	...
	// PHẦN SỬA LỖI: ĐĂNG KÝ TẤT CẢ CÁC NÚT CHO MỌI CLASS
	// Chúng ta sẽ dùng vòng lặp để đăng ký tất cả các nút từ 0 đến 15 
	// với mã 0xFF (dành cho mọi class) để đảm bảo chúng hoạt động.
	for (int i = 0; i <= 15; ++i)
	{
		RegisterBtnCharacter(0xFF, i);
	}
}
...
bool SEASON3B::CNewUIMuHelper::UpdateMouseEvent()
{
	...
	// Nếu cửa sổ skill setting đang mở, ưu tiên xử lý nó trước
	if (m_pSkillSettingWnd && m_pSkillSettingWnd->IsVisible())
	{
		if (m_pSkillSettingWnd->UpdateMouseEvent() == false)
		{
			return false; // Nuốt sự kiện nếu cửa sổ con đã xử lý
		}
	}

	// Xử lý nút đóng cửa sổ chính
	...
	// --- XỬ LÝ CÁC CHECKBOX ---
	// Lấy ID của checkbox được click
	int iButtonClickedID = UpdateMouseBoxList();
	if (iButtonClickedID != -1)
	{
		CheckBoxTap* pCheckBox = &m_CheckBoxList[iButtonClickedID];

		// Ra lệnh cho "bộ não" auto dựa trên ID của checkbox
		switch (iButtonClickedID)
		{
		case 0: // Ví dụ: Auto Potion
			gMuHelper.SetAutoPotion(pCheckBox->box->GetBoxState());
			break;
		case 1: // Ví dụ: Auto Heal
			gMuHelper.SetAutoHeal(pCheckBox->box->GetBoxState());
			break;
			// Thêm các case khác cho các checkbox còn lại...
		}
		return false; // Nuốt sự kiện
	}

	// --- XỬ LÝ CÁC NÚT BẤM (BUTTON) ---
	int iBtnID = UpdateMouseBtnList();
	if (iBtnID != -1)
	{
		switch (iBtnID)
		{
		case 2: // Nút "Setting" cho skill
		case 3:
			if (m_pSkillSettingWnd) { m_pSkillSettingWnd->Show(true); }
			break;
		case 14: // Nút Save Settings
			// Thêm logic lưu cài đặt ở đây
			break;
		case 15: // Nút Exit (X ở dưới)
			g_pNewUISystem->Hide(SEASON3B::INTERFACE_MUHELPER);
			break;
		}
		return false;
	...
	// Nếu click trong vùng cửa sổ nhưng không trúng nút nào, vẫn nuốt sự kiện
