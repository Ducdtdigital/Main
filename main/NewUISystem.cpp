...
#include "TradeX.h"
#include "Console.h" 
#include "NewUIMuHelper.h" // Thêm include
CNewUISystem* g_pNewUISystem;
CNewUIMuHelper g_pNewUIMuHelper; // Định nghĩa biến toàn cục
...
{
	Console.Write(1, "[UI Load] -> Bat dau LoadMainSceneInterface()...");
	...
	Console.Write(1, "[UI Load] OK: Basic interfaces initialized.");

	m_pNewItemMng = new CNewUIItemMng;
	Console.Write(1, "[UI Load] OK: ItemMng created.");
	...
	{
		Console.Write(1, "[UI Load] FAILED at: CNewUIChatInputBox->Create()");
		return false;
	}
	Console.Write(1, "[UI Load] OK: ChatInputBox created.");
	...
	{
		Console.Write(1, "[UI Load] FAILED at: CNewUIHotKey->Create()");
		return false;
	}
	Console.Write(1, "[UI Load] OK: HotKey created.");

	m_pNewUIMuHelper = new CNewUIMuHelper;
	int posX = GetWindowsX - 190;
	if (m_pNewUIMuHelper->Create(m_pNewUIMng, posX, 0) == false)
	{
		Console.Write(1, "[UI Load] FAILED at: CNewUIMuHelper->Create()");
		return false;
	}
	Console.Write(1, "[UI Load] OK: MuHelper created.");

	m_pNewMainFrameWindow = new CNewUIMainFrameWindow;
	if (m_pNewMainFrameWindow->Create(m_pNewUIMng, m_pNewUI3DRenderMng) == false)
	{
		Console.Write(1, "[UI Load] FAILED at: CNewUIMainFrameWindow->Create()");
		return false;
	}
	Console.Write(1, "[UI Load] OK: MainFrameWindow created.");

	m_pNewSkillList = new CNewUISkillList;
	if (m_pNewSkillList->Create(m_pNewUIMng, m_pNewUI3DRenderMng) == false)
	{
		Console.Write(1, "[UI Load] FAILED at: CNewUISkillList->Create()");
		return false;
	}
	Console.Write(1, "[UI Load] OK: SkillList created.");

	m_pNewFriendWindow = new CNewUIFriendWindow;
	if (m_pNewFriendWindow->Create(m_pNewUIMng) == false) { Console.Write(1, "[UI Load] FAILED at: CNewUIFriendWindow->Create()"); return false; }
	Console.Write(1, "[UI Load] OK: FriendWindow created.");

	m_pNewMyInventory = new CNewUIMyInventory;
	if (false == m_pNewMyInventory->Create(m_pNewUIMng, m_pNewUI3DRenderMng, GWidescreen.WidescreenPosX1, 0)) { Console.Write(1, "[UI Load] FAILED at: CNewUIMyInventory->Create()"); return false; }
	Console.Write(1, "[UI Load] OK: MyInventory created.");
	...
	Console.Write(1, "[UI Load] -> LoadMainSceneInterface() KET THUC THANH CONG.");
	return true;
...
void SEASON3B::CNewUISystem::Show(DWORD dwKey)
{
	...
	if(m_pNewUIMng) 
	{
		...
		else if (dwKey == SEASON3B::INTERFACE_MUHELPER)
		{
			...
			g_ConsoleDebug->Write(MCD_NORMAL, "[MuHelper] CNewUISystem::Show(INTERFACE_MUHELPER) called.");
			HideAllGroupA();
